public class detRedux {
    ...
    public double detByReduction () {
        ...
        for (int i = 0; i < onMatrix.column - 1; i++) {
            boolean isValid = true;
            if (onMatrix.getElement(i+1, i+1) == 0) {
                isValid = false;
                for (int search_index = i+1; search_index < onMatrix.row; search_index++) {
            ...
            for (int j = i+1; j < onMatrix.row; j++) {
